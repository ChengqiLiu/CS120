...
def DownloadFile(ftp, remotepath, localpath):        
    ...
    ftp.retrbinary('RETR {}'.format(remotepath), fp.write, 1024)
    fp.close()

def FTPConnect(host, port, username, password,p):
    ftp = FTP()
...
while True:
    ...
    if l_in[0]=="USER":
        if len(l_in)>1:
            username=l_in[1]
            str_s="Now username is: "+username
            Send(p,str_s)
        else:
            username=""
            str_s="Now username is empty."
            Send(p,str_s)
        continue
    elif l_in[0]=="PASS":
        if len(l_in)>1:
            password=l_in[1]
            str_s="Now password is: "+password
            Send(p,str_s)
        else:
            password=""
            str_s="Now password is empty."
            Send(p,str_s)
        continue
    elif l_in[0]=="HOST":
        if len(l_in)>1:
            host=l_in[1] 
        str_s="Now host is: "+host
        Send(p,str_s)
        continue
    elif l_in[0]=="CONNECT":
        ftp=FTPConnect(host,port,username,password,p)
        if ftp==None:
            str_s="exit"
            Send(p,str_s)
            exit()
...
while True:
    ...
    if len(l_in)==0:
        continue
    if l_in[0]=="PWD":
        str_s="Working Directory: "+ftp.pwd()
        Send(p,str_s)
        continue
    elif l_in[0]=="CWD":
        if len(l_in)>1:
            ftp.cwd(l_in[1])
            str_s="Change to: "+l_in[1]
            Send(p,str_s)
        else:
            str_s="Still in current directory."
            Send(p,str_s)
        continue
    elif l_in[0]=="LIST":
        if len(l_in)>1:
            str_s=ftp.dir(l_in[1])
        else:
            str_s=ftp.dir()
        input("Press enter to start transmission:\n")
        time.sleep(1)
        ...
        if len(l_in)>1:
            if l_in[1]=="True" or l_in[1]=="1":
                ftp.set_pasv(True)
                str_s="Enable passive mode."
                Send(p,str_s)
            else:
                ftp.set_pasv(False)
                str_s="Disable passive mode."
                Send(p,str_s)
        else:
            ftp.set_pasv(True)
            str_s="Enable passive mode."
        ...
        if len(l_in)>2:
            DownloadFile(ftp,l_in[1],l_in[2])
            input("Press enter to start transmission:\n")
            time.sleep(1)
            Send_file(p,l_in[2])
        continue
        ...
        for name in file_name:
            try:
                ftp.cwd(name)
                str_s=str_s+name+"\tis dir\n"
            except:
                str_s=str_s+name+"\tisn\'t dir\n"
            else:
                ftp.cwd('..')
        ftp.cwd('..')
        input("Press enter to start transmission:\n")
