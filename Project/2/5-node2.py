...
def Receive(P):
    ...
    for i in range(0, int(fs / chunk * record_time)):
        ...
        # t+=chunk/4
        # if t>10000 and t<19000 and Check_e(data):
        #     print("link error!")
        ...
        for s in range(64):
            ...
            if s==63:
                ...
                if mac_time>4:
                    ...
                    TH=(length_frame_iii+length_head)/mac_time/1000
                    print("NODE2 TH: "+str(TH)+" kbps")
                    # print("mac_time: "+str(mac_time)+" s")
                if t_d> 1600000000 and t_d<1700000000:
                    t_temp=t_d
        #decode data
        for j in range(i*length_frame_eee,(i+1)*length_frame_eee):
            decode_str=str()
            for k in range(8):
                data=frames[pointer:pointer+length_sig*4]
                pointer+=length_sig*4
...
def Send(P):
    ...
    for i in range(n_frame):
        ...
        for s in t_str:
            if s=='1':
                stream.write(signal_1_iii.tobytes())
            elif s=='0':
                stream.write(signal_0_iii.tobytes())
            else:
                print("error!")
                exit(1)
        #play the data
        for j in range(i*length_frame_iii,(i+1)*length_frame_iii):
...
pream=Preamble()
signal_0_iii=(np.sin(2*np.pi*f*np.arange(0,seconds_iii,1/fs))).astype(np.float32)
signal_1_iii=(-np.sin(2*np.pi*f*np.arange(0,seconds_iii,1/fs))).astype(np.float32)
signal_0_eee=(np.sin(2*np.pi*f*np.arange(0,seconds_eee,1/fs))).astype(np.float32)
signal_1_eee=(-np.sin(2*np.pi*f*np.arange(0,seconds_eee,1/fs))).astype(np.float32)

length_head=len(pream)
length_sig=len(signal_0_eee)
p_num=length_frame_eee
record_time=seconds_eee*(n_frame*8*(length_head/length_sig+length_frame_eee)+ADD)+more_time

t_receive=threading.Thread(target=Receive, args=(p,))
t_send=threading.Thread(target=Send, args=(p,))

time.sleep(1.2)#同步传输调整
t_receive.start()
t_send.start()
t_receive.join()
t_send.join()
