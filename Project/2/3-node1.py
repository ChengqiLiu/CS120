...
def Receive(P):
    ...
    for i in range(0, int(fs / chunk * record_time)):
        ...
        t+=chunk/4
        if t>9000 and t<510000 and Check(data):
            print("link error!")
...
def Send(P):
    #Read the file "INPUT1to2.bin"
    f = open("INPUT1to2.bin","rb")
    read=f.read()
